...
async function initApp() {
  ...
  appContainer.innerHTML = `
    <div class="game-container" style="justify-content: center; align-items: center; text-align: center;">
      <h1 class="animate-bob">${t('downloading')}</h1>
      <p id="loading-msg" style="margin-top: 16px; font-size: 14px;">${t('loadingCache')}</p>
  ...
  try {
    ...
    appContainer.innerHTML = `
      <div class="game-container" style="justify-content: center; align-items: center; text-align: center;">
        <h1 style="color: var(--color-incorrect);">${t('connError')}</h1>
        <p style="margin-top: 16px; font-size: 14px;">${t('connErrorDesc')}</p>
        <button class="btn" style="margin-top: 24px;" onclick="window.location.reload()">${t('retry')}</button>
      </div>
    `;
